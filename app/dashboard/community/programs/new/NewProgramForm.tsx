...
export function NewProgramForm({ presetCategory }: { presetCategory?: ProgramCategory }) {
  const t = useTranslations('communityPages');
  const [state, action] = useActionState(createProgram, INITIAL_STATE);
    ...
    <form action={action} className="space-y-5">
      ...
      <div className="flex flex-col gap-2.5">
        <label className="flex items-center gap-2.5 text-sm">
          <input type="checkbox" name="registration_open" value="true" defaultChecked className="size-4 accent-(--color-gold)" />
          {t('newProgram.registrationOpenLabel')}
        </label>
        <label className="flex items-center gap-2.5 text-sm">
          <input type="checkbox" name="requires_approval" value="true" className="size-4 accent-(--color-gold)" />
          {t('newProgram.requiresApprovalLabel')}
        </label>
      </div>

      <div className="pt-4">
        <SubmitButton variant="gold">{t('newProgram.submit')}</SubmitButton>
        <p className="text-xs text-(--color-fg-muted) mt-3">{t('newProgram.submitHint')}</p>
      </div>
    </form>
  );
}
