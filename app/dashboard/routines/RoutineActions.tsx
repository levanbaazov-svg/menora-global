  ...
  return (
    <form action={action} className="inline">
      ...
      <button
        ...
        title={enabled ? t('routine.pause') : t('routine.enable')}
      >
        {enabled ? t('routine.pauseLabel') : t('routine.resumeLabel')}
      </button>
...
export function DeleteRoutineButton({ id, label }: { id: string; label: string }) {
  ...
  return (
    <form
      ...
      className="inline"
      onSubmit={(e) => {
        if (!confirm(t('routine.deleteConfirm', { label }))) e.preventDefault();
      }}
    >
      <input type="hidden" name="routine_id" value={id} />
      <button
